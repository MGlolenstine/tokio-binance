...
impl UserDataClient {
    ...
    pub fn start_user_data_stream(&self) -> ParamBuilder<'_, '_, StartStreamParams>{
    ...
    pub fn keep_alive_user_data_stream<'a>(&self, listen_key: &'a str) -> ParamBuilder<'a, '_, KeepAliveStreamParams>{
        let Self { api_key,  url, client } = self;
        let url = url.join("/api/v3/userDataStream").unwrap();
    ...
    pub fn close_user_data_stream<'a>(&self, listen_key: &'a str) -> ParamBuilder<'a, '_, CloseStreamParams>{
        let Self { api_key, url, client } = self;
        let url = url.join("/api/v3/userDataStream").unwrap();
